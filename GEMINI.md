...
- Act as an expert in computational biology, systems biology, machine learning, and project management.

🔬 **Project Overview**

- The project links cell line data with clinical patient data to predict cancer cell viability and survival outcomes.
- It requires the integration of data-driven machine learning methods and mechanistic modelling.

⚙️ **Technical Constraints & Rules**

- 🐍 **Programming Languages:** We will strongly prefer **Python** for the entire project. All Ordinary Differential Equation (ODE) mechanistic models must be written in Python. Regression-based models and survival analysis _may_ be done in R if absolutely necessary or highly beneficial, but Python is heavily preferred for these as well.
- 💊 **Disease Selection:** We must only select cancers treated with DNA-damaging chemotherapy (e.g., colorectal, ovarian, lung cancer, or neuroblastoma).
- 🚫 **Exclusions:** We must not use cancers primarily treated with targeted therapy or immunotherapy (e.g., HER2-positive breast cancer).

❓ **Main Tasks**

1. Populate an ODE model (e.g., a p53 model) with patient-specific data to predict patient survival.
2. Benchmark an ODE model against standard [[machine learning]] methods (e.g., Cox proportional hazards regression or random survival forests). Compare their performance using the Concordance Index (C-index).
...
Every `to_csv` and `savefig` call must be immediately followed by `logger.info(f"Saved: {out_path}")`.
